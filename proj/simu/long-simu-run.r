...
nlme.slove<-function(phe.y, phe.cov, snp.mat)
{
	library(lme4);

	colnames(phe.cov)<-c("X1", "X2");
	df <- as.data.frame( cbind(UID=c(1:NROW(phe.cov)), phe.cov, scale(rowSums(snp.mat)) ));

	phe.one<-c();
	for(i in 1:NROW(phe.y))
	{
		for(j in 1:NCOL(phe.y))
		{
			if(!is.na(phe.y[i,j]))
				phe.one <- rbind(phe.one, data.frame( df[i,], j, phe.y[i,j]));

		}
	}

	colnames(phe.one)<-c("UID", "X1", "X2", "SNP", "TIME", "Y");

	est1 <- lmer (Y ~  X1 + X2 + ( 0 + TIME | UID), data=phe.one );
...
cat("formula=", model, "\n");
		...
		#mu_hat <- nlme.slove(phe$y, phe$cov, snp.mat[[i]]);

		obj.h0 <- try( longskat_est_model( phe$y , phe$cov, y.cov.time=par$y.cov.time, g.maxiter=1, intercept=T, method="REML" ));
